...
class FullAttention(nn.Module):
    ...
    def forward(self, q, k, v, decode_step=None, decode_idx=None):
        ...
        if mask is not None:
            ...
            attn_output = scaled_dot_product_attention(q, k, v, mask=mask, attn_dropout=self.attn_dropout, training=self.training)
        else:
            attn_output = scaled_dot_product_attention(q, k, v, mask=None, attn_dropout=self.attn_dropout, training=self.training)

        return view_range(attn_output, 2, 3, old_shape)
